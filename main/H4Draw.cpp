...
int main(int argc, char* argv[])
{
    ...
    if( (atoi(event.c_str()) != -1) && (atoi(event.c_str()) < int(h4Tree.GetEntries())) )
    {
      ...
      for(auto& channel : channels)
      {
        ...
        for(unsigned int jSample=0; jSample<samples->size(); ++jSample)
        {
          g -> SetPoint(jSample,times->at(jSample),samples->at(jSample));
          g -> SetPointError(jSample,0,baselineRMS);
        }
        
        g -> SetTitle(";sample time [ns];ADC");
        g -> SetMarkerStyle(20);
        g -> SetMarkerSize(0.7);
        g -> Draw("AL");
        g -> Draw("P,same");
        
        TLine* line_baseline = new TLine(bWinMin*tUnit,0.,bWinMax*tUnit,0.);
        line_baseline -> SetLineColor(kRed+2);
        line_baseline -> SetLineWidth(3);
        line_baseline -> Draw("same");
        
        TBox* box_baselineInt = new TBox(std::max(float(0.),float(bIntWinMin*tUnit)),0.-fitAmpMax/10.,std::min(float(204.6),float(bIntWinMax*tUnit)),0+fitAmpMax/10.);
        box_baselineInt -> SetFillColor(kRed);
        box_baselineInt -> SetFillStyle(3001);
        box_baselineInt -> Draw("same");
        
        TLine* line_signal = new TLine(sWinMin*tUnit,0.,sWinMax*tUnit,0.);
        line_signal -> SetLineColor(kGreen+2);
        line_signal -> SetLineWidth(2);
        line_signal -> Draw("same");
        
        TBox* box_signalInt = new TBox(std::max(float(0.),float((maxSample-sIntWinMin)*tUnit)),0.-fitAmpMax/10.,std::min(float(204.6),float((maxSample+sIntWinMax)*tUnit)),0+fitAmpMax/10.);
	...
	//        TLine* line_maxSample = new TLine(maxSample*tUnit,0.,maxSample*tUnit,0.+fitAmpMax);
	  ...
	  funcAmp->SetRange(times->at(maxSample-nLow), times->at(maxSample+nHig));
          ...
          TLine* line_maxTime = new TLine(fitTimeMax,0.,fitTimeMax,0.+fitAmpMax);
        ...
        if( leSlope > 0. )
        {
          ...
          TLine* line_thr = new TLine(leTime-10.,leThr,leTime+10.,leThr);
          line_thr -> SetLineColor(kBlue);
          line_thr -> SetLineStyle(2);
          line_thr -> Draw("same");
          
          TLine* line_leTime = new TLine(leTime,0.,leTime,0.+fitAmpMax);
        ...
        if( teSlope > 0. )
        {
          ...
          TLine* line_thr = new TLine(teTime-10.,teThr,teTime+10.,teThr);
          line_thr -> SetLineColor(kBlue);
          line_thr -> SetLineStyle(2);
          line_thr -> Draw("same");
          
          TLine* line_teTime = new TLine(teTime,0.,teTime,0.+fitAmpMax);
        ...
        if( cfSlope > 0. )
        {
          ...
          TLine* line_cfFrac = new TLine(cfTime-10.,0.+cfFrac*fitAmpMax,cfTime+10.,0.+cfFrac*fitAmpMax);
          line_cfFrac -> SetLineColor(kTeal);
          line_cfFrac -> SetLineStyle(2);
          line_cfFrac -> Draw("same");
          
          TLine* line_cfTime = new TLine(cfTime,0.,cfTime,0.+fitAmpMax);
    ...
    if( atoi(event.c_str()) == -1 )
    {
      ...
      while( h4Tree.NextEntry() && (index-stoul(run)*1e9<maxEvents || maxEvents==-1) )
      {
        ...
        for(auto& channel : channels)
        {
          ...
          for(unsigned int jSample=0; jSample<samples->size(); ++jSample)
          {
            g -> SetPoint(jSample,jSample*tUnit,samples->at(jSample)*0.25);
          }
