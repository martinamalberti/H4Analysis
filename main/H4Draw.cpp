...
int main(int argc, char* argv[])
{
	...
	for(int iT=0; iT<h4Tree.nEvtTimes; ++iT)
	  mainTree.time_stamps.push_back(h4Tree.evtTime[iT]);
	...
	auto timesWF = WF->GetTimes();
	  ...
	  //g -> SetPoint(jSample,jSample*tUnit,analizedWF->at(jSample)); 
	  g -> SetPoint(jSample,timesWF->at(jSample),analizedWF->at(jSample)); 
	  g -> SetPointError(jSample,0,baselineRMS);
	}
	...
	TBox* box_baselineInt = new TBox(std::max(float(0.),float(bIntWinMin)*tUnit),0.-fitAmpMax/10.,std::min(float(1023.)*tUnit,float(bIntWinMax)*tUnit),0+fitAmpMax/10.);
	...
	TBox* box_signalInt = new TBox(std::max(float(0.),float(maxSample-sIntWinMin))*tUnit,0.-fitAmpMax/10.,std::min(float(1023.)*tUnit,float(maxSample+sIntWinMax)*tUnit),0+fitAmpMax/10.);
