...
int main(int argc, char** argv)
{
  ...
  for(int entry = 0; entry < nEntries; ++entry)
    {
      ...
      // -- Photek as ref
      ampRef = treeVars.t_amp[(*treeVars.t_channelId)["PTK2"]] * k;
      timeRef   = treeVars.t_time[(*treeVars.t_channelId)["PTK2"]+treeVars.t_CFD] - timeTrg;
	...
	// -- select events
	if ( ampRef < minAmplitude["PTK2"] || ampRef > maxAmplitude["PTK2"]) continue;
  ...
  for (int ich = 0; ich < NCHANNELS; ich++){
    ...
    if (channelName == "PTK2" || channelName == "TRG0") continue;
